...
export const getAds = asyncHandler(async (req, res, next) => {
    ...
    const ads = await Ad.find(req.query, select).sort(sort).skip(pagination.start - 1).limit(limit)
...
export const getAd = asyncHandler(async (req, res, next) => {
    ...
    const ad = await Ad.findById(req.params.id).populate({path: "createUser", select: "firstName profile"})
